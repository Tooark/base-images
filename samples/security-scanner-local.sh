...
# Percent of the secret masked in the report (0-100): "100" = full redact,
# "80" = keeps 20% visible for triage, "0" = clear text (not recommended)
export BETTERLEAKS_REDACT="${BETTERLEAKS_REDACT:-100}"

# Hadolint
export HADOLINT_CONFIG="${HADOLINT_CONFIG:-}"
export HADOLINT_FORMAT="${HADOLINT_FORMAT:-json}"
# Gate level (report always keeps all levels): error|warning|info|style|none.
# Default is "error"; "warning" here also blocks the pipeline on warnings.
...
docker run --rm \
  -v "$WORKSPACE_DIR":/workspace \
  -v "$REPORTS_DIR":/reports \
  -e BETTERLEAKS_CONFIG -e BETTERLEAKS_BASELINE -e BETTERLEAKS_FORMAT -e BETTERLEAKS_OUTPUT -e BETTERLEAKS_NO_GIT -e BETTERLEAKS_EXIT_CODE -e BETTERLEAKS_FAIL_ON_FINDINGS -e BETTERLEAKS_REDACT \
